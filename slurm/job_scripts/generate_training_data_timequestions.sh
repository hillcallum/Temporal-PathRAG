#!/bin/bash
#SBATCH --job-name=gen_data_timeq
...
#SBATCH --output=/vol/bitbucket/%u/temporal_pathrag_logs/gen_timeq_%j.out
#SBATCH --error=/vol/bitbucket/%u/temporal_pathrag_logs/gen_timeq_%j.err

# Note: No GPU requested - this is CPU-only workload

echo "Generating Training Data for TimeQuestions Dataset"
echo "Job ID: $SLURM_JOB_ID"
echo "Node: $SLURM_JOB_NODELIST"
echo "Start time: $(date)"
echo "Dataset: TimeQuestions"
echo
...
echo "Generating TimeQuestions dataset training data (fast version)"
python -u scripts/training/generate_training_data_fast.py \
    ...
    --output-dir data/training/TimeQuestions \

...
if [[ $monitor_choice =~ ^[Yy]$ ]]; then
    ...
    # Create local logs directory with job-specific subdirectory
    ...
    echo "Copying model info"
    scp "$CLUSTER_USER@$CLUSTER_HOST:/vol/bitbucket/$CLUSTER_USER/llama_model_info.json" "$JOB_LOG_DIR/" 2>/dev/null || echo "No model info found"
